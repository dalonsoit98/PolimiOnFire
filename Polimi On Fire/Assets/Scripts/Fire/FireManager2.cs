...
public class FireManager2 : MonoBehaviour
{
    ...
    private float planeX = 120f;
    private float planeY = 0f;
    private float planeZ = 225f;

    private float spawnZInitial = -112.5f;
    ...
    void Update()
    {
        if (FindObjectOfType<PlayerMoveBuilding2>().isDead)
        {
            return;
        }
    ...
    void SpawnFireAtRandom()
    {
        ...
        Vector3 randomPos = new Vector3(Random.Range(-planeX / 2, planeX / 2), Random.Range(-planeY / 2, planeY / 2),
            Random.Range(-planeZ+spawnZInitial / 2, planeZ+spawnZInitial / 2));
