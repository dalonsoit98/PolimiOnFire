				...
				for (var i = 0; i < TotalWaypoints; i++) {
					if (waypointList [i] != null && waypointList[i].waypoint == null) {
						...
						foreach (Transform child in transform) {
							if (child.name == newWaypointName) {		waypointList[i].waypoint = child.GetComponent<Waypoint2> ();			}
						}
						if (waypointList [i].waypoint == null) {
							...
							waypointList[i].waypoint = newWaypoint.GetComponent<Waypoint2> ();
